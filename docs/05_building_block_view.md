...
| `ModelRegistryController` | controllers/model_registry_controller.py — post-training lifecycle (#74). Registers, copies weights into `<project>/models/` with a JSON sidecar, feeds the results panel, and offers "try it now". Registers nothing on a failed or stopped run, and nothing at all while `is_loading_project` is set. |
