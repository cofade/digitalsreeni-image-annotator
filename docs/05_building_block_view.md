...
| `core/dataset_split.py` | Group-aware train/val splitting (#80, ADR-044): `derive_groups` (exact from `image_slices`, name-prefix fallback), `merge_groups` (folds in curation clusters), `plan_split` (whole groups, plus the `fell_back` flag) and `assign_train_val`, which moved here from `io/export_formats.py` and stays re-exported there. Deliberately imports nothing from `core/slice_cache`, which reaches `QImage`. |
