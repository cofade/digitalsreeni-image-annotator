...
| `QProgressDialog` + cancel | Read `wasCanceled()` **before** `close()`, or dismiss with `reset()`/`hide()` instead | Qt's `closeEvent` emits `canceled()`, which is wired to the `cancel()` slot — so `close()` sets the flag itself. A `finally: progress.close()` followed by `if progress.wasCanceled()` reports **every** run as cancelled and silently discards its results. Verified on Qt 6.7/6.11; cost #69 an entire feature. |
| Class rename fan-out | `ClassController.rename_class` must re-key every name-keyed structure: `class_mapping`, `class_colors`, `class_visibility`, `keypoint_schemas`, every annotation, **and `dino_class_table` + `dino_phrase_panel`** | `_build_dino_class_configs` reads the *threshold table*, not the class list. Miss the rename there and every detection comes back tagged with a class that no longer exists, so `_commit_dino_results` drops all of them — which is why that method now returns `(committed, skipped)` and the callers must surface the second half rather than logging it. |
| Onion skin content | Default is ghosting the neighbour's **annotations** (dashed, unfilled, class colour), not its pixels; `onion.wants_image()` gates the only expensive half | Blending two adjacent photographic slices — which differ everywhere, unlike two animation cels — just makes the current one look out of focus. "What did I label there?" is the question worth answering. See [Onion-Skinning](docs/08_crosscutting_concepts.md#onion-skinning-and-the-slice-lru-issue-67). |
