...
| `core/qt_diagnostics.py` | Explains a Qt that will not import (#92, ADR-046): distribution versions from package *metadata*, every `Qt6Core.dll` on the loader search path in search order, each one's version read out of its PE resource without loading it. `qt_environment()` does the I/O, `diagnose()` is pure — so the rules test on a runner with no Conda and no Windows. The strictest member of this table: it exists *because* importing Qt failed. |
