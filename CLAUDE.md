...
| Keypoint / pose (#35) | A **pose instance** is `{"keypoints":[x,y,v]*K, "num_keypoints", "bbox", category…}` with **no `segmentation` key** — that absence is the discriminator routing area (→bbox), Detail-% (disabled), and the `draw_annotations` `elif "keypoints"` branch (placed **before** `elif "bbox"`). The per-class **schema** (`{names,skeleton,flip_idx}`) lives in `ImageAnnotator.keypoint_schemas` (one per class, COCO rule), embedded on each `.iap` `classes[]` entry (auto-round-trips via `convert_to_serializable`; malformed dropped on load). `KeypointTool` accepts **both mouse buttons** (right=occluded v1) so it short-circuits the left-only press dispatch like `sam_points`. Point editing reuses the #40 handle path, **not** double-click: a single-point drag uses `editing_keypoint`, and the instance box uses a new edit `kind="kpt"` that transforms the whole pose (`_scale_keypoints`/`_translate_keypoints`); both commit via `keypointEditCommitted`→`commit_keypoint_edit`. **Merge and cross-schema change-class are blocked** for instances (merge would silently delete them). K is locked once instances exist. Pure validation in `core/keypoint_schema.py`; clamp via `utils.clamp_keypoints`. See [ADR-029](docs/09_architecture_decisions.md#adr-029-keypoint--pose-annotation--per-class-schema-coco-instance-model-3-state-visibility). PR-2 (COCO/YOLO-pose export/import) + PR-3 (YOLO-pose training) still pending. |
