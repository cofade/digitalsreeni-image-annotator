...
| Qt import failure (#92) | `main.py` order is **torch (ADR-017) → guarded PyQt6 import (ADR-046) → everything else**. The guard wraps **only** the `PyQt6` import, never `annotator_window`, and routes to `core/qt_diagnostics` — which must never import Qt or `LoadLibrary` a DLL (it reads PE version resources off disk). `sreeni-cli doctor` is the same engine, and is the only diagnosis available when the loader kills the process instead of raising. The `PyQt6` cap in `pyproject.toml` is a **tested ceiling, not the fix** — don't tighten it to `<6.9` | `0xc0000139` on `import PyQt6.QtCore` is a Conda env whose own `Qt6Core.dll` / `msvcp140.dll` shadows the wheel's, not a bad PyQt6 release: CI is green unpinned on 6.11 across the matrix. Swapping the torch/Qt import order re-breaks ADR-017, reproducing only on Windows with torch installed. Wrapping `annotator_window` too would report a missing shapely as a Qt failure. See [ADR-046](docs/09_architecture_decisions.md). |
