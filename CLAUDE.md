...
| Stacks/videos are their slices | A stack or video entry in `all_images` is **not an image**: it contributes its slice/frame names. Count and export those (`task_inference.trainable_image_names`) | Counting the parent reported a video with 27 annotated frames as "0 of 1 image(s)", and a blanket "videos cannot be trained" pre-flight refused datasets the exporter handles fine. |
